...
function addPost(data) {
  let sql =
    "Insert into posts (title, topic, topic_desc, author_id) values ('" +
    data.subject +
    "','" +
    data.topic +
    "','" +
...
function getAll(data) {
  let sql =
    "Select * from posts LEFT JOIN users ON posts.author_id = users.id LEFT JOIN replies ON posts.pk_post_id = replies.post_id ORDER BY posts.date DESC";
  return db.execute(sql);
}
...
function getReplies(post_id) {
  let sql = "Select reply from users where id = " + post_id;
  return db.execute(sql);
}

function addReplyDB(data) {
  let sql =
    "Insert into replies (post_id, author_id, post_desc) values ('" +
    data.topic +
    "','" +
    data.author +
    "','" +
...
function getAllbyKeyword(keyword) {
  let combinedTable =
    "Select * from posts LEFT JOIN users ON posts.author_id = users.id LEFT JOIN replies ON posts.pk_post_id = replies.post_id";
