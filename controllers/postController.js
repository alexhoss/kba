...
exports.getAllPosts = (req, res, next) => {
    ...
    Posts.then(([rows, fieldData]) => {
        ...
        userData.getUserByID(id).then(([rows, fieldData])=>{
            console.log(rows)
            let userData = rows
            res.render('all_posts', {people: rows, peoplesCSS: true, curtisCSS:true, userData: userData[0], people: allPostData, postNumber: postNumber, messageNumber: messageNumber});
        })        
    });
};//sdsdsd
...
exports.getAllPostsHome = (req, res, next) => {
    ...
    Posts.then(([rows, fieldData]) => {
        ...
        userData.getUserByID(id).then(([rows, fieldData])=>{
            console.log(rows)
            let userData = rows
            res.render('home_johnny', {people: rows, peoplesCSS: true, curtisCSS:true, userData: userData[0], people: allPostData, postNumber: postNumber, messageNumber: messageNumber});
        })        
    });
};
