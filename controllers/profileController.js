...
exports.postSignupProfile = (req, res, next) => {
    ...
    userCheckByEamil(userData.email).then(data => {
        if (!data.length) {
            res.render('registration_curtis', { pageTitle: 'KnowledgeBase', curtisCSS: true, userData: userData })
        } else {
            let errorMessage = "email already exists!"
            res.render('login_curtis', { pageTitle: 'KnowledgeBase', curtisCSS: true, errorSignupMessage: errorMessage })
...
exports.postLoginProfile = (req, res, next) => {
    ...
    userLogin(data.email, data.pwd).then(data => {
        if (data.length) {
            ...
            res.render('login_curtis', { pageTitle: 'KnowledgeBase', curtisCSS: true, errorLoginMessage: errorMessage })
            console.log("alert to browser")
        }
    })
}
...
exports.getPeople = (req, res, next) => {
    ...
    People.then(([data, metadata]) => {
        ...
        userData.getPostsByUserID2(data[0].id).then(([rows, fieldData]) => {
            ...
            res.render('people', { people: data[0], peopleProfileCSS: true, curtisCSS: true, postNumber: postNumber, likesnumber: likesnumber, postsByUserData: postsByUserData, postsByUserDataHasData: postsByUserData.length > 0 });
        })
    });
}
